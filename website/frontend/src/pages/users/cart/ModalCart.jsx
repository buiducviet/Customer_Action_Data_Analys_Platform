...
const ModalCart = ({ show, handleClose, cartItems, total, user }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      PurchaseProduct(formDataToSend.items, String(user.id));
        ...
        <div>
          <div className="listItem">
            ...
            {cartItems.map((cartItem) => (
              <div key={cartItem.name} className="item">
                <img className="cartItemImage" src={cartItem.imageUrl} alt="#" />
                <div className="itemDetail">
                  <div className="detail">
                    ...
                    <p>Chi tiết: {cartItem.variantStorage}</p>
                    <p>Số lượng: {cartItem.quantity}</p>
                  </div>
                  <div className="price">
                    <p className="itemPrice">{formatNumber(cartItem.price)} ₫</p>
                  </div>
                </div>
              </div>
            ))}
            <h2 className="totalPrice">Total: {formatNumber(total)} đ</h2>
          </div>
